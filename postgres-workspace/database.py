...
engine = create_engine(
    f"postgresql+psycopg2://postgres:{password}@localhost/red30", echo=False
)
...
with Session(engine) as session:
    ...
    select_recent = session.execute(select(Sales).filter_by(order_num=1105910)).scalar()
    print(select_recent.cust_name)
